...
A network-based currency exchange office system built with **Windows Communication Foundation (WCF)** on the .NET Framework. The system retrieves real exchange rates from the **National Bank of Poland (NBP) API** and lets users register accounts, manage balances, and buy/sell currencies through a WPF desktop client.

This project was developed for the **Network Application Development** course (Labs 1–15).
...
3. Start the service host. Either run it from Visual Studio, or from an **administrator command prompt**:
   ...
   CurrencyHost.exe
   ```
   Wait for `Service is running at: http://localhost:8080/CurrencyService`.
4. Run the **CurrencyWpfClient** project (Ctrl+F5).
5. In the client window:
   - Enter a username and password, then click **Register**.
   - Use **Top Up** to add PLN funds.
   - Click **USD / EUR / GBP** or enter a code and click **Get Rate**.
   - Use **Buy** / **Sell** to trade currencies.
   - Click **History** to view past transactions.
...
Developed for educational purposes as part of a university course project.
